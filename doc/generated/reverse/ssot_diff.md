...
|------|--------------|-----------|-------------------|
| Next.js | 15 | 16 | **16.1.6** |

**検証結果**: `apps/web/package.json` で `"next": "16.1.6"` を確認。実際のコードはNext.js 16を使用。

**提案**: RDDの「Next.js 15」を「Next.js 16」に更新（SSOTルールにより、この変更にはユーザー承認が必要）

**ステータス**: ⚠️ RDD更新待ち（CLAUDE.md・リバースドキュメントは16に修正済み）
...
| 2026-02-02 | Next.jsバージョン再検証（実際は16.1.6）、RDD更新待ちに変更 |
