/**
 * スペース別の洞察一覧APIエンドポイント
 *
 * GET /api/spaces/:id/insights
 * - 指定したスペースに紐づく洞察をFirestoreから取得する
 * - createdAt昇順でソート（新しいものが下＝ページの時系列に合わせる）
 *
 * 対話別の洞察取得（/api/conversations/:id/insights）のスペース版。
 */
 ...
 * @param context - ルートコンテキスト（params.id: スペースID）
...
export async function GET(
  ...
  try {
    ...
    return createServerErrorResponse(error, 'GET /api/spaces/:id/insights');
  }
}
